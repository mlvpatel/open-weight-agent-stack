...
- **Branch protection on `main`** was verified on 2026-08-11 to require `diagrams`, `generated`, `invariants`, `html`, and `links`; force pushes and branch deletion are disabled. `browser` and CodeQL cannot become required locally: the owner must add them after their first hosted runs exist.
- **Current single-maintainer trade-offs.** Required signatures, administrator enforcement, linear history, and conversation resolution are currently off. CODEOWNERS records the owner locally, but GitHub only enforces its review requirement after branch protection enables code-owner review. Enabling those settings reduces bypass paths but can create a maintainer lockout; this is an owner decision, not a completed control.
...
- **CodeQL workflow configuration** is committed for the repository's Python and JavaScript sources on pushes, pull requests, weekly schedules, and manual dispatch. Hosted code scanning is not claimed as active until this workflow is pushed and completes on GitHub.
- **A Content-Security-Policy is delivered** as a meta element, since Pages cannot send headers. `default-src 'none'` is the baseline. The vendored `mermaid.min.js` is allowed from this origin via `script-src 'self'`, and the single inline bootstrap is pinned by hash. The hash does not restrict other same-origin scripts. On `*.github.io` that origin is the Pages subdomain for the whole account, so a second project published there shares it. `frame-ancestors`, `report-uri` and `sandbox` are ignored in meta form per the spec, so clickjacking protection and violation reporting remain unavailable.

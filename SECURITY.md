...
- **A Content-Security-Policy is delivered** as a meta element, since Pages cannot send headers. It denies everything by default, pins the single inline script by hash, and blocks external connections. `frame-ancestors`, `report-uri` and `sandbox` are ignored in meta form per the spec, so clickjacking protection and violation reporting remain unavailable.
